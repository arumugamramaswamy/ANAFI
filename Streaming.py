...
import olympe.messages.camera as cam
from olympe.messages.camera import set_camera_mode,set_photo_mode,take_photo,set_recording_mode
...
class StreamingExample:
    ...
    def __init__(self):
        ...
        self.drone(set_recording_mode(0,0,4,0,0)).wait()
        input()
        self.tempd = tempfile.mkdtemp(prefix="olympe_streaming_test_")
        # print(cam.recording_capabilities)
    ...
    def yuv_frame_cb(self, yuv_frame):
        ...
        if isinstance(ids,np.ndarray) and list(ids) and ids[0]==0:
            # t = time.clock()
            # self.drone.stop_video_streaming().wait()
            # self.drone(set_camera_mode(0,1)).wait()
            # self.drone(set_photo_mode(0,0,1,0,0,0,0)).wait()
            # self.drone(take_photo(cam_id=0)).wait()
            # print(time.clock()-t)
            # self.drone(set_camera_mode(0,0)).wait()
            # self.drone.start_video_streaming().wait()
            print("Saving file {}...".format(self.count))
            cv2.imwrite("Images/"+str(self.count)+".jpg",cv2frame)
            self.count+=1
